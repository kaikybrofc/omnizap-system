...
export const getQuoteModuleConfig = () => runtime.getModuleConfig();

export const resolveQuoteCommandName = (command) => runtime.resolveCommandName(command);
export const getQuoteCommandEntry = (command) => runtime.getCommandEntry(command);
export const listEnabledQuoteCommands = () => runtime.listEnabledCommands();

export const getQuoteTextConfig = () => {
  const config = getQuoteModuleConfig();
...
export const getQuoteUsageText = (command, { commandPrefix = '/', header, variant } = {}) => {
  const entry = getQuoteCommandEntry(command);
  const methods = resolveUsageLines(entry, variant);
  if (!methods.length) return '';

  const prefixHeader = typeof header === 'string' ? header : getQuoteTextConfig().usage_header || DEFAULT_TEXTS.usage_header;
