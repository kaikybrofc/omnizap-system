...
export const getRpgPokemonModuleConfig = () => runtime.getModuleConfig();

export const resolveRpgPokemonCommandName = (command) => runtime.resolveCommandName(command);
export const getRpgPokemonCommandEntry = (command) => runtime.getCommandEntry(command);
export const listEnabledRpgPokemonCommands = () => runtime.listEnabledCommands();

export const getRpgPokemonTextConfig = () => {
  const config = getRpgPokemonModuleConfig();
...
export const getRpgPokemonUsageText = (command, { commandPrefix = '/', header, variant } = {}) => {
  const entry = getRpgPokemonCommandEntry(command);
  const methods = resolveUsageLines(entry, variant);
  if (!methods.length) return '';

  const prefixHeader = typeof header === 'string' ? header : getRpgPokemonTextConfig().usage_header || DEFAULT_TEXTS.usage_header;
