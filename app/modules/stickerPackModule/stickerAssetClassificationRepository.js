...
const normalizeClassificationRow = (row) => {
  ...
  return {
    ...
    entropy,
    entropy_normalized:
      entropy !== null ? Number(deriveEntropyNormalized(entropy, topLabels).toFixed(6)) : null,
    confidence_margin:
      row.confidence_margin !== null && row.confidence_margin !== undefined
        ? Number(row.confidence_margin)
        : null,
    affinity_weight:
      row.affinity_weight !== null && row.affinity_weight !== undefined
        ? Number(row.affinity_weight)
        : null,
    nsfw_score:
      row.nsfw_score !== null && row.nsfw_score !== undefined ? Number(row.nsfw_score) : null,
...
export async function upsertStickerAssetClassification(payload, connection = null) {
  await executeQuery(
    ...
    [
      ...
      payload.llm_pack_suggestions
        ? JSON.stringify(payload.llm_pack_suggestions)
        : JSON.stringify([]),
...
export async function listStickerClassificationsForDeterministicReprocess(
  { limit = 250, cursorAssetId = '', entropyThreshold = 0.8, affinityThreshold = 0.3 } = {},
  connection = null,
) {
