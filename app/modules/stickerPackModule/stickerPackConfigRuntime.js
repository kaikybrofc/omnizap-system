...
export const getStickerPackModuleConfig = () => runtime.getModuleConfig();

export const resolveStickerPackCommandName = (command) => runtime.resolveCommandName(command);
export const getStickerPackCommandEntry = (command) => runtime.getCommandEntry(command);
export const listEnabledStickerPackCommands = () => runtime.listEnabledCommands();

export const getStickerPackTextConfig = () => {
  const config = getStickerPackModuleConfig();
...
export const getStickerPackUsageText = (command, { commandPrefix = '/', header, variant } = {}) => {
  const entry = getStickerPackCommandEntry(command);
  const methods = resolveUsageLines(entry, variant);
  if (!methods.length) return '';

  const prefixHeader = typeof header === 'string' ? header : getStickerPackTextConfig().usage_header || DEFAULT_TEXTS.usage_header;
