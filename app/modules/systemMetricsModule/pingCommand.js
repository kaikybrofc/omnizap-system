...
const formatDateTime = (date = new Date()) =>
  `${padNumber(date.getDate())}/${padNumber(date.getMonth() + 1)}/${date.getFullYear()} ${padNumber(date.getHours())}:${padNumber(date.getMinutes())}:${padNumber(date.getSeconds())}`;
...
const buildPingMessage = ({
  ...
  const responseTime = Number.isFinite(latencyMs)
  ...
  const loadPerCore =
    Number.isFinite(load1) && systemMetrics.totalCpus > 0 ? load1 / systemMetrics.totalCpus : null;
...
`.trim();
  ...
  const queueValues = [
    metricsSummary.queues.messages,
    metricsSummary.queues.chats,
    metricsSummary.queues.lid_map,
  ]
