...
export const getTikTokModuleConfig = () => runtime.getModuleConfig();

export const resolveTikTokCommandName = (command) => runtime.resolveCommandName(command);
export const getTikTokCommandEntry = (command) => runtime.getCommandEntry(command);
export const listEnabledTikTokCommands = () => runtime.listEnabledCommands();

export const getTikTokTextConfig = () => {
  const config = getTikTokModuleConfig();
...
export const getTikTokUsageText = (command, { commandPrefix = '/', header, variant } = {}) => {
  const entry = getTikTokCommandEntry(command);
  const methods = resolveUsageLines(entry, variant);
  if (!methods.length) return '';

  const prefixHeader = typeof header === 'string' ? header : getTikTokTextConfig().usage_header || DEFAULT_TEXTS.usage_header;
