...
function StatusPanel() {
    ...
    const load = async () => {
      ...
      try {
        ...
        setState({
          ...
          cpu: Number.isFinite(Number(host.cpu_percent))
            ? `${Number(host.cpu_percent).toFixed(2)}%`
            : 'n/d',
          ram:
  ...
  return h(
    ...
    h(
      'h3',
      { className: 'sub-title' },
      h(Icon, { cls: 'fa-solid fa-heart-pulse' }),
    ...
    h(
      ...
      state.latencyMs
        ? h('span', { className: 'status-meta' }, `Latência: ${state.latencyMs}ms`)
        : null,
    ),
    h(
      'div',
      ...
      h(
        ...
        h('strong', null, 'CPU host'),
        h('span', null, state.cpu || 'n/d'),
      ),
      h(
        'div',
        { className: 'status-item' },
        h('strong', null, 'RAM host'),
        h('span', null, state.ram || 'n/d'),
      ),
      h(
        'div',
        { className: 'status-item' },
        h('strong', null, 'Uptime processo'),
        h('span', null, state.uptime || 'n/d'),
      ),
    ),
...
function ApiDocsApp() {
  return h(
    ...
    h(
      'p',
      null,
      'Documentação técnica da API OmniZap para bots e automação WhatsApp, com o catálogo de stickers como módulo integrável.',
    ),
