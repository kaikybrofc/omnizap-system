    ...
    <div className="min-h-screen bg-[#020617] text-white font-sans selection:bg-primary selection:text-primary-content overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-[#020617]/60 backdrop-blur-xl transition-all">
        <div className="container mx-auto px-4">
          ...
          <nav className="flex lg:hidden items-center gap-2 overflow-x-auto no-scrollbar pb-3 -mx-1 px-1">${NAV_ITEMS.map((item) => html` <a href=${item.href} className="btn btn-ghost btn-xs rounded-lg font-bold text-white/40 whitespace-nowrap bg-white/5"> ${item.label} </a> `)}</nav>
        </div>
      </header>

      <main className="relative z-10">
        <!-- Hero Section -->
        <section className="relative overflow-hidden py-12 lg:py-32 min-h-[80vh] flex items-center">
          <div className="container mx-auto px-4 relative z-10">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div data-reveal="fade-right" className="text-center lg:text-left space-y-8">
                ...
                <h1 className="text-4xl sm:text-6xl lg:text-8xl font-black leading-[1.05] tracking-tighter text-balance">
                  Automatize seu <br />
                ...
                <p className="text-lg sm:text-xl text-white/50 max-w-xl mx-auto lg:mx-0 leading-relaxed font-medium">O bot mais poderoso do Brasil para gerenciar grupos, criar figurinhas e automatizar fluxos inteligentes via IA.</p>

                <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4">
                  <a href=${botMenuUrl} className="btn btn-primary btn-lg rounded-2xl shadow-2xl shadow-primary/30 gap-3 group px-8 h-16 text-lg font-black">
                    Adicionar ao WhatsApp
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                  </a>
                  <a href="/comandos/" className="btn btn-outline btn-lg rounded-2xl border-white/10 hover:bg-white/5 hover:border-primary/40 px-8 h-16 text-lg font-black gap-2 transition-all"> <span>⚡</span> Ver Comandos </a>
                </div>

                <div className="flex flex-wrap justify-center lg:justify-start gap-12 pt-10">
                  <div className="relative group">
                    <div className="text-4xl sm:text-5xl font-black text-white group-hover:text-primary transition-colors">${metrics.commands}</div>
                    <div className="text-[10px] uppercase font-black text-white/30 tracking-[0.3em] mt-1">Comandos Vivos</div>
                  </div>
                  <div className="w-px h-12 bg-white/5 hidden sm:block"></div>
                  <div>
                    <div className="text-4xl sm:text-5xl font-black text-white">${metrics.users}</div>
                    <div className="text-[10px] uppercase font-black text-white/30 tracking-[0.3em] mt-1">Usuários</div>
                  </div>
                  <div className="w-px h-12 bg-white/5 hidden sm:block"></div>
                  <div>
                    <div className="text-4xl sm:text-5xl font-black text-white/80">${metrics.messages}</div>
                    <div className="text-[10px] uppercase font-black text-white/30 tracking-[0.3em] mt-1">Requisições</div>
                  </div>
                </div>
              </div>
                ...
                <div className="relative bg-white/[0.03] backdrop-blur-2xl rounded-[2rem] border border-white/10 p-6 shadow-2xl animate-float">
                  ...
                  <div className="space-y-4 font-mono text-sm sm:text-base">
                    <div className="flex gap-3">
                      ...
                      <span className="text-white/60">"Como faço stickers?"</span>
                    </div>
                    <div className="flex gap-3 bg-primary/10 p-4 rounded-xl border border-primary/20">
                      ...
                      <span className="text-primary/90 italic">"Envie uma imagem com o comando /s e eu faço o resto! ✨"</span>
                    </div>
                    <div className="flex gap-3">
                      <span className="text-primary font-bold">visitor:</span>
                      <span className="text-white/60">"E pra moderar o grupo?"</span>
                    </div>
                    <div className="flex gap-3 bg-emerald-500/5 p-4 rounded-xl border border-emerald-500/10">
                      <span className="text-info font-bold">omnizap:</span>
                      <span className="text-white/80">"Use /captcha ou /ban. Proteção 24/7 ativa. 🛡️"</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-start">
              ...
              <div className="w-full lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="lg:hidden mt-2">
                  <a href="/comandos/" className="btn btn-block btn-outline border-white/10 rounded-2xl h-14 font-black text-[10px] uppercase tracking-widest"> Ver Todos os ${metrics.commands} Comandos </a>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-24 container mx-auto px-4">
          <div data-reveal="fade-up" className="relative p-12 sm:p-20 rounded-[3rem] overflow-hidden text-center text-primary-content shadow-[0_0_80px_rgba(34,197,94,0.1)]">
            ...
            <div className="relative z-10 space-y-8">
              ...
              <div className="pt-4">
                <a href=${botMenuUrl} className="btn btn-lg bg-white border-none text-[#020617] rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-2xl px-12 h-16 text-lg font-black uppercase tracking-widest"> Adicionar Agora </a>
              </div>
            </div>
          </div>
        </section>
      ...
      <a href=${botMenuUrl} target="_blank" className="hidden lg:flex btn btn-primary btn-circle fixed bottom-8 right-8 z-50 w-16 h-16 shadow-[0_0_40px_rgba(34,197,94,0.3)] hover:scale-110 active:scale-95 transition-all border-none group">
