...
export const createGoogleWebAuthRuntime = ({
  executeQuery,
  runSqlTransaction,
  tables,
  logger,
  ...
  const normalizedConfiguredCookiePath = normalizeBasePath(
    configuredSessionCookiePath,
    normalizeBasePath(process.env.STICKER_WEB_GOOGLE_SESSION_COOKIE_PATH, '/'),
  );
  const legacyCookiePaths = Array.from(
    new Set([
      normalizedConfiguredCookiePath,
      normalizeBasePath(apiBasePath, '/api/sticker-packs'),
      `${normalizeBasePath(apiBasePath, '/api/sticker-packs')}/auth`,
      normalizeBasePath(webPath, '/stickers'),
      normalizeBasePath(loginWebPath, '/login'),
    ]),
  );
